...
/* ──────────────────────────────────────────────
   NEW — COLUMN CLEARANCE (BOX-AWARE)
────────────────────────────────────────────── */
function computeColumnBounds(
  rects: Map<string, Rect>,
  layers: Map<string, number>
): Map<number, ColumnBounds> {
  const bounds = new Map<number, ColumnBounds>();
  ...
  rects.forEach((r, id) => {
    ...
    if (!b) {
      bounds.set(col, { minY: top, maxY: bottom });
    } else {
...
function clampLaneYToColumnClearance(
  desiredY: number,
  fromCol: number,
  toCol: number,
  columnBounds: Map<number, ColumnBounds>
): number {
  const minCol = Math.min(fromCol, toCol);
  const maxCol = Math.max(fromCol, toCol);

  let safeAbove = -Infinity;
  let safeBelow = Infinity;

  for (let c = minCol; c <= maxCol; c++) {
    const b = columnBounds.get(c);
    if (!b) continue;

    safeAbove = Math.max(safeAbove, b.minY - LANE_SPACING);
    safeBelow = Math.min(safeBelow, b.maxY + LANE_SPACING);
  }

  if (desiredY < safeAbove) return safeAbove;
  if (desiredY > safeBelow) return safeBelow;

  const distUp = Math.abs(desiredY - safeAbove);
  const distDown = Math.abs(desiredY - safeBelow);

  return distUp < distDown ? safeAbove : safeBelow;
}
...
export default function LeaderLinesOverlay({
  ...
  const renderedEdges: Edge[] = useMemo(() => {
    ...
    const columnBounds = computeColumnBounds(rects, layers);
    const out: Edge[] = [];
    ...
    for (const { fromId, toId } of links) {
      ...
      const allocated = allocateLaneYForGap(
        biasedY,
        gapKey,
        claimedByGap
      );

      const laneY = clampLaneYToColumnClearance(
        allocated,
        fromCol,
        toCol,
        columnBounds
